...
- 网页端支持直接选择 Provider、选择模型预设、手动覆盖模型名，并提供 API Key 粘贴框
- 将结果写入本地 PostgreSQL
...
Grafana 现在默认开启匿名只读查看，所以任务完成后自动弹出的链接会直接进入仪表盘；如果你要做管理配置，仍可使用下面的管理员账号：
...
所以后续测试时，你既可以把默认值写进 `.env`，也可以在网页里临时切换成另一种模型或另一把 Key。
...
- 或 `IKUNCODE_BASE_URL` + `IKUNCODE_MODEL` + `IKUNCODE_API_KEY`
